...
func MakeUnderLineInView(target: UIView) -> UIView {
    let lineView = UIView(frame: CGRect(x: 5, y: target.frame.height - 10, width: target.frame.size.width, height: 1.8))
...
func MakeUnderLineInStackView(target: UIStackView) -> UIStackView {
    let lineView = UIStackView(frame: CGRect(x: 5, y: -10, width: target.frame.size.width, height: 1.8))
    lineView.backgroundColor = #colorLiteral(red: 1, green: 0.674518168, blue: 0, alpha: 1)
    return lineView
}
